tellraw @s ["", {"text": "Done. Players can join the runners team by running /trigger JoinRunners command. To add them manually yourself, click here or run /execute as <Player> run trigger JoinRunners. If random runners option is enabled, it's not needed to add runners manually", "clickEvent": {"action": "suggest_command", "value": "/execute as MatinButNot run trigger JoinRunners"}}, {"text": "\nWhen done, click here or run /function nothunt:preparation/start to start the match. Enjoy!", "clickEvent": {"action": "suggest_command", "value": "/function nothunt:preparation/start"}}]
